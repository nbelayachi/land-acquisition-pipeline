...
class CompleteDashboardGenerator:
    ...
    def create_ownership_complexity_charts(self):
        ...
        try:
            ...
            if not owners_data.empty:
                ...
                very_complex = sum(complexity_counts.get(i, 0) for i in range(6, 100)) # Increased range
                
                values = [simple, moderate, complex_3_5, very_complex]
                total_parcels_in_chart = sum(values)

                # CORRECTED and more robust average calculation
    ...
    def create_b2b_b2c_charts(self):
        ...
        fig = go.Figure(data=[
            go.Pie(
                labels=[f'B2B Corporate (PEC)<br>{b2b_area:.1f} Ha ({b2b_count} parcels)', 
                       f'B2C Individual (Mail)<br>{b2c_area:.1f} Ha ({b2c_count} parcels)'],
        ...
        fig.update_layout(
            title=dict(
                text="<b>B2B vs. B2C Campaign Segmentation</b><br><sub>Corporate (PEC) vs. Individual (Mailing) Outreach</sub>",
                x=0.5,
                font=dict(size=14)
            ),
            height=400,
        ...
        <body>
            <div class="main-container">
                <div class="header">
                    ...
                    <p><strong>Campaign Success: {stages['Final_Mailing']['parcels']}/{stages['Input_File']['parcels']} parcels ({pipeline_success:.1f}%) | Mailed Area: {stages['Final_Mailing']['area']:.1f} hectares | Area Retention: {area_retention:.1f}%</strong></p>
