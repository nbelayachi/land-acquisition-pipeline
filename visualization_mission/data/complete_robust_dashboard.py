...
class CompleteDashboardGenerator:
    ...
    def load_data(self):
        ...
        try:
            ...
            data['Owners_Normalized'] = pd.read_excel(self.excel_path, sheet_name='Owners_Normalized')
    ...
    def create_enhanced_dual_funnel(self):
        ...
        fig = make_subplots(
            rows=1, cols=2,
            subplot_titles=("Technical Processing", "Value Progression"),
        ...
        fig.update_layout(
            title=dict(
                text="<b>Dual Funnel Analysis</b><br><sub>Technical Processing vs Value Progression</sub>",
                x=0.5,
                font=dict(size=16)
            ),
            height=500,
    ...
    def create_enhanced_geographic_chart(self):
        ...
        fig.update_layout(
            title=dict(
                text="<b>Mailings Geographic Distribution</b><br><sub>Final Mails Distribution by Municipality</sub>",
                x=0.5,
                font=dict(size=14)
            ),
            height=500,
    ...
    def create_enhanced_area_flow_chart(self):
        ...
        fig.update_layout(
            title=dict(
                text="<b>Area Flow Analysis</b><br><sub>Hectare Progression -- Incorrect input data returns no data!</sub>",
                x=0.5,
                font=dict(size=14)
            ),
            height=450,
    ...
    def create_ownership_complexity_charts(self):
        """Ownership complexity analysis, calculated from the 'Owners_Normalized' sheet."""
        categories = ['Simple (1 owner)', 'Moderate (2 owners)', 'Complex (3-5 owners)', 'Very Complex (6+ owners)']
        values = [0, 0, 0, 0]
        total_parcels_in_chart = 0
        avg_owners = 0

        try:
            owners_data = self.data['Owners_Normalized']
            if not owners_data.empty:
                owners_data['unique_parcel_id'] = (
                    owners_data['comune'].astype(str) + '-' +
                    owners_data['foglio_input'].astype(str) + '-' +
                    owners_data['particella_input'].astype(str)
                )
                parcel_owner_counts = owners_data.groupby('unique_parcel_id').size()
                complexity_counts = parcel_owner_counts.value_counts().sort_index()
                ...
                very_complex = sum(complexity_counts.get(i, 0) for i in range(6, 100))

                values = [simple, moderate, complex_3_5, very_complex]
                total_parcels_in_chart = sum(values)

                if total_parcels_in_chart > 0:
                    total_owners_sum = sum(k * v for k, v in complexity_counts.items())
                    avg_owners = total_owners_sum / total_parcels_in_chart

        except KeyError:
            print("Warning: 'Owners_Normalized' sheet not found. Ownership complexity chart will be empty.")

        fig = go.Figure(data=[
            go.Pie(
                labels=categories,
                values=values,
                hole=0.4, # Standard hole size
                marker=dict(colors=['#16a34a', '#d97706', '#dc2626', '#7c2d12'], line=dict(color='white', width=2)),
                textinfo='label+percent',
        ...
        fig.update_layout(
            title=dict(
                text="<b>Ownership Complexity Distribution</b><br><sub>Parcel Categorization by Owner Count. Contains ALL parcels for which landowners information was retrieved (incl. owners without residential Cat.A properties</sub>",
                x=0.5,
                font=dict(size=16)
            ),
            height=600, # A good height for a full-width chart
            annotations=[
                dict(
                    text=f"<b>{total_parcels_in_chart}</b><br>Total Parcels<br><b>{avg_owners:.1f}</b><br>Avg Owners",
                    x=0.5, y=0.5,
    ...
    def create_b2b_b2c_charts(self):
        """
        B2B/B2C segmentation with corrected logic.
        B2B and B2C are now treated as two separate, mutually exclusive campaigns.
        """
        b2b_area = 0
        b2b_count = 0
        b2c_area = self.robust_metrics['stages']['Final_Mailing']['area']
        b2c_count = self.robust_metrics['stages']['Final_Mailing']['parcels']
        
        try:
            companies_data = self.data['All_Companies_Found']
            if not companies_data.empty:
                b2b_count = len(companies_data)
                if companies_data['Area'].dtype == 'object':
                    companies_data['Area'] = companies_data['Area'].astype(str).str.replace(',', '.').astype(float)
                b2b_area = companies_data['Area'].sum()
        except KeyError:
            print("Warning: 'All_Companies_Found' sheet not found. B2B data will be zero.")

        fig = go.Figure(data=[
            go.Pie(
                labels=[f'Company (PEC)<br>{b2b_area:.1f} Ha ({b2b_count} parcels)', 
                       f'Individual (Mail)<br>{b2c_area:.1f} Ha ({b2c_count} parcels)'],
        ...
        total_parcels = b2b_count + b2c_count
        
        fig.update_layout(
            title=dict(
                text="<b>B2B vs. B2C Campaign Segmentation</b><br><sub>Companies (PEC) vs. Individual (Mailing) Outreach</sub>",
                x=0.5,
                font=dict(size=14)
            ),
            height=400,
            annotations=[
                dict(
                    text=f"<b>{total_area:.1f} Ha</b><br>Total Campaign Area<br><b>{total_parcels}</b><br>Total Parcels",
                    x=0.5, y=0.5,
                    font=dict(size=12, color=self.colors['primary']),
                    showarrow=False,
                    bgcolor="rgba(255,255,255,0.8)",
    ...
    def create_address_quality_distribution(self):
        """Address quality distribution chart based on real data only."""
        labels = []
        values = []
        ...
        try:
            ...
            print("Warning: 'Address_Quality_Distribution' sheet not found. Chart will be empty.")
        ...
        fig = go.Figure(data=[
            go.Pie(
                ...
                values=values,
                hole=0.4,
                pull=[0.1] + [0] * (len(values)-1) if len(values) > 1 else ([0.1] if len(values) == 1 else []),
    ...
    def create_ownership_complexity_charts(self):
        """Ownership complexity analysis, calculated from the 'Owners_Normalized' sheet."""
        categories = ['Simple (1 owner)', 'Moderate (2 owners)','Complex (3-5 owners)', 'Very Complex (6+ owners)']
        values = [0, 0, 0, 0] # Start with zero values
        total_parcels_in_chart = 0
        avg_owners = 0
        ...
        fig = go.Figure(data=[
            go.Pie(
                labels=categories,
                values=values,
                hole=0.3, # <-- MODIFIED: Smaller hole makes the chart look bigger
                marker=dict(colors=['#16a34a', '#d97706', '#dc2626', '#7c2d12'], line=dict(color='white', width=2)),
                textinfo='label+percent',
                textfont=dict(size=12), # Slightly larger font
        ...
        fig.update_layout(
            ...
            height=550, # A reasonable height
            margin=dict(l=30, r=30, t=80, b=30), # <-- MODIFIED: Reduced margins to give the pie more space
            annotations=[
                dict(
                    text=f"<b>{total_parcels_in_chart}</b><br>Total Parcels<br><b>{avg_owners:.1f}</b><br>Avg Owners",
                    x=0.5, y=0.5,
    ...
    def create_municipality_performance_table(self):
        ...
        fig = go.Figure(data=[go.Table(
            header=dict(
                values=['<b>Municipality</b>', '<b>Input Parcels</b>', '<b>API Success (%)</b>', '<b>Final Success (%)</b>', '<b>Final Parcels</b>', '<b>Target Area (Ha)</b>'],
        ...
        fig.update_layout(
            title=dict(
                text="<b>Municipality Performance Summary</b><br><sub>API & Final Success Rates | Target Area by Municipality</sub>",
                x=0.5,
                font=dict(size=14)
            ),
            height=350,
    ...
    def create_corporate_opportunities_table(self):
        """Corporate opportunities table based on real data only."""
        company_summary = []
        ...
        try:
            ...
            if not companies_data.empty:
                ...
                # Group by company name to aggregate parcels and area
                if name_cols:
                    ...
                    company_groups = companies_data.groupby(name_col).agg(
                        parcel_count=('foglio_input', 'count'),
                        area=('Area', 'sum'),
                        municipality=('comune_input', 'first')
                    ).reset_index()

                    company_groups = company_groups.sort_values('area', ascending=False).head(10)

                    for _, row in company_groups.iterrows():
                         pec_status = '✅ Available' # Simplified
            ...
            print("Warning: 'All_Companies_Found' sheet not found. Corporate opportunities table will be empty.")

        # Create a figure with a message if the summary is empty
        if not company_summary:
            fig = go.Figure()
            fig.add_annotation(text="No company data available to display.", xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False, font=dict(size=16))
        else:
            fig = go.Figure(data=[go.Table(
                header=dict(
        ...
        fig.update_layout(
            title=dict(
                text="<b>Companies Opportunities Table</b><br><sub>Top B2B targets sorted by area potential</sub>",
                x=0.5,
                font=dict(size=14)
            ),
            height=350,
    ...
    def create_process_efficiency_metrics(self):
        ...
        fig.update_layout(
            title=dict(
                text="<b style='color: #1e293b; font-family: Inter;'>Process Efficiency Dashboard</b><br><sub style='color: #64748b; font-size: 12px;'>High level KPIs</sub>",
                x=0.5,
    ...
    def generate_complete_executive_dashboard(self):
        ...
        stages = self.robust_metrics['stages']
        pipeline_success = stages['Final_Mailing']['parcels'] / stages['Input_File']['parcels'] * 100 if stages['Input_File']['parcels'] > 0 else 0
        area_retention = stages['Final_Mailing']['area'] / stages['Input_File']['area'] * 100 if stages['Input_File']['area'] > 0 else 0
        
        # Calculate ratios for insights
        final_parcels = stages['Final_Mailing']['parcels']
        final_area = stages['Final_Mailing']['area']
        final_owners = len(self.robust_metrics['final_mailing'])
        ...
        <head>
            ...
            <title>Campaign #4 Results Dashboard</title>
            ...
            <style>
                * {{
                ...
                body {{
                    ...
                    margin: 0;
                    padding: 0;
                    background: linear-gradient(135deg, #6ee7b7 0%, #15803d 100%);
                    min-height: 100vh;
                    color: #1e293b;
                }}
                ...
                .chart-container {{ animation: slideInUp 0.6s ease-out; }}
                @keyframes slideInUp {{
        ...
        <body>
            <div class="main-container">
                <div class="header">
                    ...
                    <h1>Campaign #4 - Results</h1>
                    <p>Castiglione - Casalpusterlengo | Lombardy </p>
                    <p><strong>Mailed Success: {stages['Final_Mailing']['parcels']}/{stages['Input_File']['parcels']} parcels ({pipeline_success:.1f}%) | Mailed Area: {stages['Final_Mailing']['area']:.1f} ha</strong></p>
                    ...
                    <div style="display: flex; justify-content: center; gap: 30px; margin-top: 20px; flex-wrap: wrap;">
                        <div style="text-align: center; padding: 15px; border-radius: 12px;">
                            ...
                            <div style="font-size: 0.9rem; color: #4b5563;">per Mailed Parcel</div>
                        </div>
                        <div style="text-align: center; padding: 15px; border-radius: 12px;">
                        ...
                        <div style="text-align: center; padding: 15px; border-radius: 12px;">
                            ...
                            <div style="font-size: 0.9rem; color: #4b5563;">per Contacted Owner</div>
                        </div>
                        <div style="text-align: center; padding: 15px; border-radius: 12px;">
                            <div style="font-size: 1.4rem; font-weight: 600; color: #d97706;">{final_owners}</div>
                            <div style="font-size: 0.9rem; color: #4b5563;">Total Owners Contacted</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="dashboard-grid">
                    ...
                    <div class="chart-container">{efficiency_html}</div>
                    <div class="dual-section">
                        <div class="chart-container">{geographic_html}</div>
                        <div class="chart-container">{area_flow_html}</div>
                    </div>
                    <div class="chart-container">{municipality_html}</div>
                    ...
                    <div class="dual-section">
                        <div class="chart-container">{b2b_html}</div>
                        <div class="chart-container">{quality_html}</div>
                    </div>
                </div>
                <footer>N.B.</footer>
            </div>
