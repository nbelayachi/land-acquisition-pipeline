...
def validate_enhanced_funnel_output(results_file_path):
    ...
    try:
        ...
        print(f"  Old Funnel_Analysis: {'FOUND (should be replaced)' if has_old_funnel else 'REPLACED'}")
    ...
    try:
        ...
        if len(land_funnel) > 0 and len(contact_funnel) > 0:
            ...
            if land_efficiency >= 80:
                ...
                print(f"  Good land retention ({land_efficiency:.1f}% kept through filters)")
            else:
            ...
            if contact_multiplication >= 2.5:
                ...
                print(f"  Limited contact expansion ({contact_multiplication:.1f}x addresses per qualified parcel)")
            
            if zero_touch_rate >= 15:
                print(f"  High automation potential ({zero_touch_rate:.1f}% zero-touch ready)")
            elif zero_touch_rate >= 10:
                print(f"  Moderate automation potential ({zero_touch_rate:.1f}% zero-touch ready)")
            else:
...
def main():
    ...
    results_file = r"C:\Projects\land-acquisition-pipeline\completed_campaigns\LandAcquisition_Casalpusterlengo_Castiglione_20250702_1150\LandAcquisition_Casalpusterlengo_Castiglione_20250702_1150_Results.xlsx"
    ...
    if validation_success:
        ...
        print(f"\nVALIDATION FAILED - Issues need to be resolved before commit")
